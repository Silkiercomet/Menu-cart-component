                ...
                <section>
                    ...
                    {cart.map((item: ItemCart) => <div className={style.cart__item} key={item.product.name}>
                        <div className={style.cart__item_details}>
                            ...
                            <p><span>{item.amount}x</span> <span>@ ${parseFloat(String(item.product.price)).toFixed(2)}</span>
                                <span>${parseFloat(String(item.amount * item.product.price)).toFixed(2)}</span></p>
                        </div>
                        <button onClick={() => removeItem(item.id)}><img src={"./assets/images/icon-remove-item.svg"}
                                                                         alt={"remove item"}/></button>
                    </div>)}
                    <div className={style.cart__total}><span>order total</span> <span>${parseFloat(String(total_price)).toFixed(2)}</span></div>
                </section>
